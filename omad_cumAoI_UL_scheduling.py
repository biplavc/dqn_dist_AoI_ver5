...
def find_omad_cumAoI_UL_action(eval_env, slot): ## maximal age difference
    ...
    last_packet_generated = {}

    for i in eval_env.UAV_age:
        if current_slot%eval_env.periodicity[i]==0: # generate at will has period 1 so %=0
            last_packet_generated[i] = eval_env.current_step  # generation time of the packet sampled now
            ...
            last_packet_generated[i] = eval_env.periodicity[i]*max(math.floor(eval_env.current_step/eval_env.periodicity[i]), 1) ## max added so that the result is never 0 as our slots start from 1. so periodicity of 2 means 2,4,6,8
    ...
    for i in eval_env.UAV_age:
        UAV_age_if_sampled[i] = current_slot - last_packet_generated[i] ## if sampled at current slot, UAV_age

    UAV_cum_age = {}
    for i in UAV_age_if_sampled:
        cum_age = 0
        for j in eval_env.dest_age:
            if j[0] == i:
                cum_age = cum_age + (eval_env.dest_age[j] - UAV_age_if_sampled[i])
            UAV_cum_age[i] = cum_age
    ...
    # if verbose:
    #     print(f"UAV_cum_age = {UAV_cum_age}")
    ...
    ## select UL users start
    upload_users = []
    sampleDict_copy = copy.deepcopy(UAV_cum_age)

    while len(upload_users) < len(eval_env.UAV_age):

        ## listOfKeys_cumAoI has the users with highest cum AoI
        itemMaxValue = max(sampleDict_copy.items(), key=lambda x: x[1])
        listOfKeys_cumAoI = list()
        for key, value in sampleDict_copy.items():
            if value == itemMaxValue[1]:
                listOfKeys_cumAoI.append(key)
                        
        ## listOfKeys_DL will have the number of DLs attached to users having same cumAoI
        listOfKeys_DL = {}
        for i in listOfKeys_cumAoI:
            listOfKeys_DL[i] = DL[i]
            
        ## find the user with the minimum DL and add it to upload users
        itemMinValue = min(listOfKeys_DL.items(), key=lambda x: x[1])
        listOfKeys = list() ## users with min DL 
        for key, value in listOfKeys_DL.items():
        ...
        if verbose:
            print(f"DL_copy = {DL_copy}")
                
        if remaining_capacity > 0:
            upload_users.extend(random.sample(listOfKeys, len(listOfKeys)))
            if verbose:
                print(f"remaining_capacity = {remaining_capacity}, upload_users = {upload_users}")
        for items in listOfKeys:
            del sampleDict_copy[items]
          
        if verbose:    
            print(f"UAV_cum_age = {UAV_cum_age}, listOfKeys_cumAoI = {listOfKeys_cumAoI}, listOfKeys_DL = {listOfKeys_DL}, listOfKeys = {listOfKeys}")    
    ...
    if verbose:
        print(f"\n\nIMPTT - tx_users = {eval_env.tx_users}, tx_rx_pairs = {eval_env.tx_rx_pairs}, UAV_age = {eval_env.UAV_age}, UAV_cum_age = {UAV_cum_age}, dest_age = {eval_env.dest_age}, upload_users = {upload_users}, non_upload_users = {non_upload_users}, excluded_pairs = {excluded_pairs}, non_excluded_pairs = {non_excluded_pairs}") ## working
...
def omad_cumAoI_UL_scheduling(I, drones_coverage, folder_name, deployment, packet_upload_loss_thresh, packet_download_loss_thresh, periodicity, adj_matrix, tx_rx_pairs, tx_users, RB_needed_UL, RB_needed_DL, BS_location, user_locations, MAX_STEPS):  ## maximal age difference
    ...
    print(f"\nomad_cumAoI_UL_scheduling started for {I} users , coverage = {drones_coverage} with packet_upload_loss_thresh = {packet_upload_loss_thresh}, packet_download_loss_thresh = {packet_download_loss_thresh}, periodicity = {periodicity}, tx_rx_pairs = {tx_rx_pairs}, tx_users = {tx_users},  RB_needed_UL = {RB_needed_UL}, RB_needed_DL = {RB_needed_DL}  and {deployment} deployment", file = open(folder_name + "/results.txt", "a"), flush = True)
    ...
    for ep in range(random_episodes): # how many times the random policy will be run, similar to episode
        if ep%log_interval_random ==0 and ep!=0:
            ...
            print(f"oMAD cum AoI ep = {ep} took {time_gap} seconds from start, rate = {round((1/rate),2)} eps/sec, finish_time = {finish_time} ", flush = True)
        ep_reward = 0
        ep_peak_reward = 0
        ...
        if ep==0:
            print(f"\noMAD cum AoI scheduling and {deployment} placement with {I} users, coverage is {eval_env.act_coverage}, RB_total_UL is {RB_total_UL}, RB_total_DL = {RB_total_DL}, action space size is {eval_env.action_size} \n\n", file = open(folder_name + "/action_space.txt", "a"), flush = True)
            
            print(f"\noMAD cum AoI scheduling and {deployment} placement with {I} users, coverage is {eval_env.act_coverage}, RB_total_UL is {RB_total_UL}, RB_total_DL = {RB_total_DL}, action space size is {eval_env.action_size} ", file = open(folder_name + "/results.txt", "a"), flush = True)
        ...
        for x in range(MAX_STEPS):
            ...
            upload_users, download_user_pairs = find_omad_cumAoI_UL_action(eval_env, eval_env.current_step)


            if verbose:
                print(f"\n\ncurrent episode = {ep}\n")
                print(f"slot = {eval_env.current_step} for OMAD cum AoI begins, upload_users = {upload_users}, download_user_pairs = {download_user_pairs}, BS_age = {eval_env.UAV_age}, dest_age = {eval_env.dest_age}\n")                
    ...
    pickle.dump(best_episodes_average, open(folder_name + "/" + deployment + "/" + str(I) + "U_omad_waoi_episodes_average.pickle", "wb"))
    pickle.dump(best_episodes_peak, open(folder_name + "/" + deployment + "/" + str(I) + "U_omad_waoi_best_episodes_peak.pickle", "wb"))
    
    pickle.dump(overall_ep_peak_reward, open(folder_name + "/" + deployment + "/" + str(I) + "U_omad_waoi_overall_ep_peak_reward.pickle", "wb"))    
    pickle.dump(final_step_peak_rewards, open(folder_name + "/" + deployment + "/" + str(I) + "U_omad_waoi_final_step_peak_rewards.pickle", "wb"))
    
    pickle.dump(overall_ep_reward, open(folder_name + "/" + deployment + "/" + str(I) + "U_omad_waoi_overall_ep_reward.pickle", "wb"))
    pickle.dump(final_step_UAV_rewards, open(folder_name + "/" + deployment + "/" + str(I) + "U_omad_waoi_final_step_UAV_rewards.pickle", "wb"))
    pickle.dump(final_step_rewards, open(folder_name + "/" + deployment + "/" + str(I) + "U_omad_waoi_final_step_rewards.pickle", "wb"))
    
    pickle.dump(age_dist_UAV, open(folder_name + "/" + deployment + "/" + str(I) + "U_omad_waoi_age_dist_UAV.pickle", "wb"))
    pickle.dump(age_dist_dest, open(folder_name + "/" + deployment + "/" + str(I) + "U_omad_waoi_age_dist_dest.pickle", "wb"))
    
    pickle.dump(eval_env.sample_time, open(folder_name + "/" + deployment + "/" + str(I) + "U_omad_waoi_sample_time.pickle", "wb"))
    
    pickle.dump(success_upload, open(folder_name + "/" + deployment + "/" + str(I) + "U_omad_waoi_success_upload.pickle", "wb"))
    pickle.dump(success_download, open(folder_name + "/" + deployment + "/" + str(I) + "U_omad_waoi_success_download.pickle", "wb"))
    pickle.dump(attempt_upload, open(folder_name + "/" + deployment + "/" + str(I) + "U_omad_waoi_attempt_upload.pickle", "wb"))
    pickle.dump(attempt_download, open(folder_name + "/" + deployment + "/" + str(I) + "U_omad_waoi_attempt_download.pickle", "wb"))
    
    pickle.dump(unutilized_RB_DL, open(folder_name + "/" + deployment + "/" + str(I) + "U_omad_waoi_unutilized_RB_DL.pickle", "wb"))
    pickle.dump(unutilized_RB_UL, open(folder_name + "/" + deployment + "/" + str(I) + "U_omad_waoi_unutilized_RB_UL.pickle", "wb"))
    
    pickle.dump(mad_UL_schedule, open(folder_name + "/" + deployment + "/" + "omad_waoi_UL_schedule_" + str(I) + "U.pickle", "wb"))    
    pickle.dump(mad_DL_schedule, open(folder_name + "/" + deployment + "/" + "omad_waoi_waoi_DL_schedule_" + str(I) + "U.pickle", "wb"))  
    
    print("\noMAD cum AoI scheduling ", deployment, " placement, ", I, " users, MAX_STEPS = ", MAX_STEPS, ", MEAN of overall_ep_reward = ", np.mean(overall_ep_reward), " MIN and MAX of overall_ep_reward = ", np.min(overall_ep_reward),", ", np.max(overall_ep_reward),  " ... MEAN of overall_ep_peak_reward = ", np.mean(overall_ep_peak_reward), " MIN and MAX of overall_ep_peak_reward = ", np.min(overall_ep_peak_reward),", ", np.max(overall_ep_peak_reward), ". Similarly for final_step_UAV_rewards - MEAN = ",{np.mean(final_step_UAV_rewards)}, ", MIN and MAX of final_step_UAV_rewards = ", np.min(final_step_UAV_rewards),", ", np.max(final_step_UAV_rewards)," end with final state of ", eval_env._state, " with shape ", np.shape(eval_env._state), ", min PDR_upload = ", {np.min(PDR_upload)}, ", max PDR_upload = ", {np.max(PDR_upload)}, ", min PDR_download = ", {np.min(PDR_download)}, ", max PDR_upload = ", {np.max(PDR_download)}, ", max_total_packet_lost_upload = ", np.max(total_packet_lost_upload), ", min_total_packet_lost_upload = ", np.min(total_packet_lost_upload), ", max_total_packet_lost_download = ", np.max(total_packet_lost_download), ", min_total_packet_lost_download = ", np.min(total_packet_lost_download))
    ...
    print(f"oMAD cum AoI best_episodes_average = {sorted_best_episodes_average}", file = open(folder_name + "/best_episodes.txt", "a"), flush = True)
    print(f"\n\n\n", file = open(folder_name + "/best_episodes.txt", "a"), flush = True)
    print(f"oMAD cum AoI sorted_best_episodes_peak = {sorted_best_episodes_peak}", file = open(folder_name + "/best_episodes.txt", "a"), flush = True)
    print(f"\n\n\n ------------------------------------------------", file = open(folder_name + "/best_episodes.txt", "a"), flush = True)
