...
def find_lrb_action(eval_env, slot): ## maximal age difference
    ...
    ## random select DL pairs start ____________

    # lrb_age_dest = {tuple(x):(eval_env.dest_age[tuple(x)] - eval_env.UAV_age[x[0]]) for x in eval_env.tx_rx_pairs}
    
    ## pending start
    sampleDict_copy_1 = copy.deepcopy(eval_env.RB_pending_DL)
    sampleDict_copy_2 = copy.deepcopy(eval_env.RB_needed_DL)
        
    lrb_user_pairs = []
    while len(sampleDict_copy_1) !=0:
        
        if verbose:
            print(f"sampleDict_copy_1 = {sampleDict_copy_1}, lrb_user_pairs = {lrb_user_pairs}", flush=True)
        
        itemMaxValue = min(sampleDict_copy_1.items(), key=lambda x: x[1], default=0)
        listOfKeys = list()
        listOfKeys_1 = list()
        for key, value in sampleDict_copy_1.items():
            if value == itemMaxValue[1] and itemMaxValue[1]!=0:
                listOfKeys.append(key)
            elif itemMaxValue[1]==0:
                listOfKeys_1.append(key)

        remaining_capacity = len(eval_env.tx_rx_pairs) - len(lrb_user_pairs)
        if remaining_capacity > 0 and len(listOfKeys)!=0:
            lrb_user_pairs.extend(random.sample(listOfKeys, len(listOfKeys)))
        for items in listOfKeys:
            del sampleDict_copy_1[items]
        for items in listOfKeys_1:
            del sampleDict_copy_1[items]
            
    if verbose:
        print(f"pending DL lrb_user_pairs={lrb_user_pairs}", flush=True)
            
    ## pending end
    
    ## new start, combine with pending
    while len(lrb_user_pairs) < len(eval_env.tx_rx_pairs):
    
        itemMaxValue = min(sampleDict_copy_2.items(), key=lambda x: x[1])
        listOfKeys = list()
        for key, value in sampleDict_copy_2.items():
    ...
    if verbose:
        print(f"pending and new DL lrb_user_pairs_actual={lrb_user_pairs_actual}", flush=True)
    ...
    ## random select UL pairs start

    ## pending start
    sampleDict_copy_1 = copy.deepcopy(eval_env.RB_pending_UL)
    sampleDict_copy_2 = copy.deepcopy(eval_env.RB_needed_UL)
    ...
    if verbose:
        print(f"pending UL upload_users={upload_users}", flush=True)
    ...
    if verbose:
        print(f"pending and new UL upload_users={upload_users}", flush=True)
       
    ## new end

    assert len(upload_users)==RB_total_UL and len(download_user_pairs_arr)==RB_total_DL
