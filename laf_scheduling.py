...
def find_laf_action(eval_env, slot): ## maximal age difference
    ...
    laf_age_dest = eval_env.dest_age 
    sampleDict_copy = copy.deepcopy(laf_age_dest)
    
    laf_user_pairs = []
    while len(laf_user_pairs) < len(laf_age_dest):
        ...
        remaining_capacity = len(laf_age_dest) - len(laf_user_pairs)
        if remaining_capacity > 0:
            laf_user_pairs.extend(random.sample(listOfKeys, len(listOfKeys)))
        for items in listOfKeys:
            del sampleDict_copy[items]
            
    # print(f"download_user_pairs_arr for MAF = {laf_user_pairs}")
    laf_user_pairs.reverse()
    # print(f"download_user_pairs_arr for LAF = {laf_user_pairs}")            
            
    laf_user_pairs_actual = laf_user_pairs[:eval_env.RB_total_DL]
            
    ## random select DL pairs end
    
    download_user_pairs = laf_user_pairs_actual
    ...
    if verbose:
        print(f"\nslot {slot} begins. laf age is {laf_age_dest}, age at dest is {eval_env.dest_age}, age at UAV is {eval_env.UAV_age}.")  
 ...
 ############################################
    ...
    # actual_laf_action = None # this has to change
    ...
    #         actual_laf_action = action

    #         if verbose:
    #             print(f"upload_users={upload_users}, download_user_pairs={download_user_pairs_arr}, actual_laf_action = {actual_laf_action}")    
                
    #         break
        
    # assert actual_laf_action!=None
        
    return upload_users, download_user_pairs_arr     


def laf_scheduling(I, drones_coverage, folder_name, deployment, packet_upload_loss_thresh, packet_download_loss_thresh, periodicity, adj_matrix, tx_rx_pairs, tx_users, RB_needed_UL, RB_needed_DL, BS_location, user_locations, MAX_STEPS):  
        
    # print(f"\nlaf started for {I} users , coverage = {drones_coverage} with update_loss_thresh = {packet_upload_loss_thresh}, packet_download_loss_thresh = {packet_download_loss_thresh}, periodicity = {periodicity}, tx_rx_pairs = {tx_rx_pairs}, tx_users = {tx_users}, RB_needed_UL = {RB_needed_UL}, RB_needed_DL = {RB_needed_DL} and {deployment} deployment", flush = True)
    
    print(f"\nlaf started for {I} users , coverage = {drones_coverage} with packet_upload_loss_thresh = {packet_upload_loss_thresh}, packet_download_loss_thresh = {packet_download_loss_thresh}, periodicity = {periodicity}, tx_rx_pairs = {tx_rx_pairs}, tx_users = {tx_users},  RB_needed_UL = {RB_needed_UL}, RB_needed_DL = {RB_needed_DL}  and {deployment} deployment", file = open(folder_name + "/results.txt", "a"), flush = True)
    ...
    laf_UL_schedule = {} ## key:([slot 1st DL completed, gen time of the packet], [slot 2nd DL completed, gen time of the packet], [...], [...])
    for i in tx_users:
        laf_UL_schedule[i] = [] ## list corresponding to each key

    laf_DL_schedule = {} ## key:([slot 1st UL completed, gen time of the packet], [slot 2nd UL completed, gen time of the packet], [...], [...])
    for i in tx_rx_pairs:
        laf_DL_schedule[tuple(i)] = [] ## list corresponding to each key  
    ...
    for ep in range(random_episodes): # how many times the random policy will be run, similar to episode
        ...
        if ep==0:
            print(f"\nlaf scheduling and {deployment} placement with {I} users, coverage is {eval_env.act_coverage}, RB_total_UL is {RB_total_UL}, RB_total_DL = {RB_total_DL}, action space size is {eval_env.action_size} \n\n", file = open(folder_name + "/action_space.txt", "a"), flush = True)
            
            print(f"\nlaf scheduling and {deployment} placement with {I} users, coverage is {eval_env.act_coverage}, RB_total_UL is {RB_total_UL}, RB_total_DL = {RB_total_DL}, action space size is {eval_env.action_size} ", file = open(folder_name + "/results.txt", "a"), flush = True)
        ...
        for x in range(MAX_STEPS):
            # print(x)
            # print("inside laf - ", x, " step started")      ## runs MAX_STEPS times       
            # selected_action = find_laf_action(eval_env, eval_env.current_step)
            ...
            upload_users, download_user_pairs = find_laf_action(eval_env, eval_env.current_step)
                        ...
                        if eval_env.RB_pending_DL[tuple(i)] == 0: # means packet was fully downloaded in this slot
                            ...
                            if eval_env.curr_DL_gen[tuple(i)] == -1: ## current packet started DL when BS had nothing 
                                ...
                                if (random_episodes-ep)<100: ##means the last 100 episodes
                                    laf_DL_schedule[tuple(i)].append([ep, eval_env.current_step + delay_from_throughput*delay_include, eval_env.curr_DL_gen[tuple(i)]])
                    ...
                    if eval_env.RB_pending_UL[i]==0: ## this user has completed the prev packet in its prev attempt, switch to a new packet now. both sample and try to send the new packet. also for the first slot, this wil be true                  
                        ...
                        if eval_env.RB_pending_UL[i] == 0: # means packet was fully uploaded in this slot
                            ...
                            if eval_env.curr_UL_gen[i] == -1: ## current packet started UL when device had nothing 
                                ...
                                if (random_episodes-ep)<100: ##means the last 100 episodes
                                    laf_UL_schedule[i].append([ep, eval_env.current_step + delay_from_throughput*delay_include, eval_env.curr_UL_gen[i]])
                            # new packet done
    ...
    # print(f"laf_UL_schedule = {laf_UL_schedule}, laf_DL_schedule = {laf_DL_schedule}")
    
    pickle.dump(best_episodes_average, open(folder_name + "/" + deployment + "/" + str(I) + "U_laf_episodes_average.pickle", "wb"))
    pickle.dump(best_episodes_peak, open(folder_name + "/" + deployment + "/" + str(I) + "U_laf_best_episodes_peak.pickle", "wb"))
    
    pickle.dump(overall_ep_peak_reward, open(folder_name + "/" + deployment + "/" + str(I) + "U_laf_overall_ep_peak_reward.pickle", "wb"))    
    pickle.dump(final_step_peak_rewards, open(folder_name + "/" + deployment + "/" + str(I) + "U_laf_final_step_peak_rewards.pickle", "wb"))
    
    pickle.dump(overall_ep_reward, open(folder_name + "/" + deployment + "/" + str(I) + "U_laf_overall_ep_reward.pickle", "wb"))
    pickle.dump(final_step_UAV_rewards, open(folder_name + "/" + deployment + "/" + str(I) + "U_laf_final_step_UAV_rewards.pickle", "wb"))
    pickle.dump(final_step_rewards, open(folder_name + "/" + deployment + "/" + str(I) + "U_laf_final_step_rewards.pickle", "wb"))

    pickle.dump(age_dist_UAV, open(folder_name + "/" + deployment + "/" + str(I) + "U_laf_age_dist_UAV.pickle", "wb"))
    pickle.dump(age_dist_dest, open(folder_name + "/" + deployment + "/" + str(I) + "U_laf_age_dist_dest.pickle", "wb"))
 
    pickle.dump(eval_env.sample_time, open(folder_name + "/" + deployment + "/" + str(I) + "U_laf_sample_time.pickle", "wb"))
    
    pickle.dump(success_upload, open(folder_name + "/" + deployment + "/" + str(I) + "U_laf_success_upload.pickle", "wb"))
    pickle.dump(success_download, open(folder_name + "/" + deployment + "/" + str(I) + "U_laf_success_download.pickle", "wb"))
    pickle.dump(attempt_upload, open(folder_name + "/" + deployment + "/" + str(I) + "U_laf_attempt_upload.pickle", "wb"))
    pickle.dump(attempt_download, open(folder_name + "/" + deployment + "/" + str(I) + "U_laf_attempt_download.pickle", "wb"))
    
    pickle.dump(unutilized_RB_DL, open(folder_name + "/" + deployment + "/" + str(I) + "U_laf_unutilized_RB_DL.pickle", "wb"))
    pickle.dump(unutilized_RB_UL, open(folder_name + "/" + deployment + "/" + str(I) + "U_laf_unutilized_RB_UL.pickle", "wb"))
    
    pickle.dump(laf_UL_schedule, open(folder_name + "/" + deployment + "/" + "laf_UL_schedule_" + str(I) + "U.pickle", "wb"))    
    pickle.dump(laf_DL_schedule, open(folder_name + "/" + deployment + "/" + "laf_DL_schedule_" + str(I) + "U.pickle", "wb")) 
    
    print("\nlaf scheduling ", deployment, " placement, ", I, " . MEAN of final_step_rewards = ", np.mean(final_step_rewards), ". MEAN of overall_ep_reward = ", np.mean(overall_ep_reward), " MIN and MAX of overall_ep_reward = ", np.min(overall_ep_reward),", ", np.max(overall_ep_reward),  " ... MEAN of overall_ep_peak_reward = ", np.mean(overall_ep_peak_reward), " MIN and MAX of overall_ep_peak_reward = ", np.min(overall_ep_peak_reward),", ", np.max(overall_ep_peak_reward), ". Similarly for final_step_UAV_rewards - MEAN = ",{np.mean(final_step_UAV_rewards)}, ", MIN and MAX of final_step_UAV_rewards = ", np.min(final_step_UAV_rewards),", ", np.max(final_step_UAV_rewards)," end with final state of ", eval_env._state, " with shape ", np.shape(eval_env._state), ", min PDR_upload = ", {np.min(PDR_upload)}, ", max PDR_upload = ", {np.max(PDR_upload)}, ", min PDR_download = ", {np.min(PDR_download)}, ", max PDR_upload = ", {np.max(PDR_download)}, ", max_total_packet_lost_upload = ", np.max(total_packet_lost_upload), ", min_total_packet_lost_upload = ", np.min(total_packet_lost_upload), ", max_total_packet_lost_download = ", np.max(total_packet_lost_download), ", min_total_packet_lost_download = ", np.min(total_packet_lost_download))
    ...
    print(f"laf best_episodes_average = {sorted_best_episodes_average}", file = open(folder_name + "/best_episodes.txt", "a"), flush = True)
    print(f"\n\n\n", file = open(folder_name + "/best_episodes.txt", "a"), flush = True)
    print(f"laf sorted_best_episodes_peak = {sorted_best_episodes_peak}", file = open(folder_name + "/best_episodes.txt", "a"), flush = True)
    print(f"\n\n\n ------------------------------------------------", file = open(folder_name + "/best_episodes.txt", "a"), flush = True)
